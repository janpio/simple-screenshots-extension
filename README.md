# Chrome Full Page Screenshot Extension

## Goal

Create a minimal Chrome extension that captures screenshots using the Chrome DevTools Protocol (same as Chrome's built-in "Capture full size screenshot" from the command palette). This avoids the issues browser extension alternatives have with fixed headers/footers.

## Requirements

- Capture **visible area** and **full page** screenshots
- Use **Chrome DevTools Protocol** (`Page.captureScreenshot`) — not scroll-based stitching
- Copy screenshot directly to **clipboard**
- Trigger via **right-click context menu** (two options) or **clicking the pinned extension icon** (defaults to full page)

## Extension Structure

```
chrome-screenshot-extension/
├── manifest.json
├── background.js
├── icon16.png
├── icon48.png
└── icon128.png
```

## Key Implementation Details

### manifest.json
- Manifest V3
- Permissions: `activeTab`, `debugger`, `contextMenus`, `notifications`, `clipboardWrite`
- Background service worker: `background.js`

### background.js — Core Logic

1. **Context menu**: Two entries — "Screenshot Visible Area" and "Screenshot Full Page"
2. **Icon click**: Defaults to full page screenshot
3. **Capture flow**:
   - Attach Chrome Debugger to active tab
   - For full page: call `Page.getLayoutMetrics` to get full content size, then `Emulation.setDeviceMetricsOverride` to resize viewport to full page dimensions
   - Call `Page.captureScreenshot` with `format: "png"` (and `captureBeyondViewport: true` for full page)
   - For full page: call `Emulation.clearDeviceMetricsOverride` to reset viewport
   - Detach debugger
4. **Clipboard**: Convert base64 screenshot to blob, use `navigator.clipboard.write()` with `ClipboardItem`
5. **Notifications**: Show success/failure via `chrome.notifications`

### Icons
- Simple placeholder icons (blue square with camera outline) can be generated with Python/Pillow or replaced with any icon set

## Installation

1. Save all files in a folder
2. Go to `chrome://extensions/`
3. Enable **Developer mode**
4. Click **Load unpacked** → select the folder

## Usage

- **Right-click** anywhere on a page → "Screenshot Visible Area" or "Screenshot Full Page"
- **Click extension icon** in toolbar → full page screenshot
- Screenshot is copied to clipboard automatically

## Notes

- The debugger attachment causes a brief "debugging this tab" banner — this is expected behavior
- DevTools Protocol approach produces identical results to Chrome's built-in screenshot command
- No scroll-based stitching means fixed headers/footers are handled correctly
